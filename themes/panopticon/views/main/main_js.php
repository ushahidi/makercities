...
jQuery(function() {
	...
	$("ul#category_switch li > a").click(function(e) {
		...
		var oldLayers = map._olMap.getLayersByName(/Reports.*/);
		map.deleteLayer(oldLayers);
		
		if(categoryId == 0) {
			var cats = <?php //error_log(var_export($this->template->content->all_categories, true));
        echo json_encode(array_keys($this->template->content->all_categories));
			?>;
			// console.log(cats);
			// map.addLayer("GeoJSON", {name:"Reports",url:"json",transform:false,category_id:"7,9"}, true, true);
		}
		else{
