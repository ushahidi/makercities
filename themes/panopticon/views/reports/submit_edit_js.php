...
if (!count($_POST)) : ?>
		...
		jQuery(window).load(function() {
			...
			// F'realz AJAX form	
			$('form#reportForm').ajaxForm({
				beforeSubmit: validateReport,
				success: function(res) {
					if (res.errors) {
						var ul = $('<ul />').html('<li>' + res.errors.join('</li><li>') +'</li>');
						ul.appendTo('#submitErrors');
					} else if (res.success == 'ok') {
						$('form#reportForm')[0].reset();
						$('#submitErrors').html('');
						if (res.id && window.top.reportClick) {
							window.top.reportClick('reports/view/' + res.id);
							window.top.zoomToLongLat($('#longitude').val(), $('#latitude').val());
							if (window.top.submitModal) window.top.submitModal(res);
						}
					}

					$('#reportForm input').removeAttr('disabled');
				},
				uploadProgress: function(event, position, total, percentComplete) {
					...
					$('#file-style').remove();
					$('<style type="text/css" id="file-style">#media-content .upload-file:before{width:' + percentComplete + '%;}</style>').appendTo('head');
					//$('#media-content .upload-file:before').css({ width: percentVal });
/*					console.log(event);
					console.log(percentComplete, position, total);*/
				}
			});

			$('.upload-button input').bind('change', uploadFileChange);

			// Submit category selection tabs
			$('#category-tabs li').click(function(){
				var li      = $(this),
					wrapper = $('#category-descriptions'),
					panelName,
					lastPanel;
				...
				// Hide all
				$('#category-descriptions div.pane').hide();

				lastPanel = $('#category-tabs li.active').removeClass('active');
				if (lastPanel.length) lastPanel = lastPanel.attr('id').match(/tab-([\w-]+)/);
				panelName = li.attr('id').match(/tab-([\w-]+)/);
        console.log(panelName);

				$('#category-' + panelName[1]).show(); // Show clicked
				$('#tab-' + panelName[1]).addClass('active');
				$('#future_category').val(li.attr('title'));
			});
			...
			$('#media-tabs li').click(function(){
				...
				// Hide all
				wrapper.find('div.pane').hide();

				lastPanel = $('#media-tabs li.active').removeClass('active');
				if (lastPanel.length) lastPanel = lastPanel.attr('id').match(/tab-([\w-]+)/);
				panelName = li.attr('id').match(/tab-([\w-]+)/);

				$('#media-' + panelName[1]).show(); // Show clicked
				$('#tab-' + panelName[1]).addClass('active');
			});
			
			$('#media-content button.pm').click(function(e) {
				var parent   = $(this).parent(),
				    newInput, newRow, clone;

				$(this).blur();

				if ($(this).hasClass('plus')) { // Add a row
					if (parent.parent().find('.item').length >= 5) {
						$(this).addClass('disabled');
						return false;
					}

					newRow   = parent.clone(true);
					newInput = newRow.find('input');
					newInput.val(newInput.attr('data-prompt'));
					newRow.find('button.pm').removeClass('plus').addClass('minus');
					newRow.appendTo(parent.parent());

					if (newRow.hasClass('file')) {
						newRow.find('.upload-file').removeClass('error').attr('data-filename', '');

						clone = $('<input type="file"></input>')
								.attr({
									name: newInput.attr('name'),
									class: newInput.attr('class'),
								})
								.bind('change', uploadFileChange);
						newInput.remove();
						newRow.find('.upload-button').append(clone);
					}
				} else { // Remove a row
					parent.parents('.pane').find('.pm.plus').removeClass('disabled');
					parent.remove();
				}
			...
			$('input[type=text], textarea').each(function(){
				var val = $(this).val();
				$(this).attr({ 'data-prompt': val, 'placeholder': val });
			});
			
			$('input[type=text], textarea').focus(function(){
				if($(this).attr('data-prompt') != $(this).val())
					return;
		...
		function uploadFileChange() {
			var input           = $(this),
			    file            = this.files[0],
			    parent          = input.parent(),
			    fileName        = input.parents('.item').find('.upload-file'),
			    isValidFileType = false,
			    validFileTypes = [ 'image/jpeg', 'image/png', 'image/gif' ];

			fileName.removeClass('error');

			for (var n = 0; n < validFileTypes.length; n++) {
			    if (file.type.indexOf(validFileTypes[n]) > -1) {
			        isValidFileType = true;
			    }
			}

			if (!isValidFileType) {
				var clone = $('<input type="file"></input>')
						.attr({
				...
				fileName.addClass('error').attr('data-filename', 'Invalid file type.');
			} else {
		...
		function validateReport(formData, jqForm, options) {
			var errors = 0;
			var firstEl = false;
			$(this).addClass('error');
			jqForm.find('input[data-prompt], textarea[data-prompt]').each(function(e) {
				var valIsPrompt = ($(this).val() == $(this).attr('data-prompt'));

				if (valIsPrompt || $(this).val() == '') {
					var minLength = $(this).attr('data-minlength') || 0;

					if ($(this).attr('data-required') == 1 || $(this).val().length < minLength ) {
						errors++;
						$(this).addClass('error');
						if (!firstEl) firstEl = $(this);
					}
				}
			});
			...
			if (firstEl) {
				firstEl.focus();
				$('html, body').animate({
					scrollTop: firstEl.offset().top - 10
				}, 500);
			}
			...
			$.post("<?php echo url::site() . 'reports/geocode/' ?>", { address: address },
				function(data){
					if (data.status == 'success'){
						...
						// display the map centered on a latitude and longitude
						// map.panTo(myPoint);
						map.setCenter(myPoint, 14, true, true);
												
						// Update form values
						$("#country_name").val(data.country);
						$("#latitude").val(data.latitude);
						$("#longitude").val(data.longitude);
						$("#location_find, #location_name").val(data.location_name);
					} else {
					...
					if (searchBox) searchBox.removeClass('loading');
