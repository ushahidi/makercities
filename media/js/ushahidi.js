...
(function(){
	...
	Ushahidi.Map.prototype.addLayer = function(layerType, options, save, keepOnTop) {
		...
		if (layerType !== Ushahidi.KML) {
			...
			if(options.category_id != null)
				params.push('c' + '=' + options.category_id);
			for (var _key in this._reportFilters) {
				if(_key == 'c' && options.category_id != null)					
					continue;
	...
	Ushahidi.Map.prototype.onFeatureSelect = function(event) {
		...
		var content = '';
		content += "<div class=\"infowindow\">" + image +
	...
	/**
	 * APIMethod: zoomToCurrentLocation
	 * Callback to be executed when zooming to the user's current location
	 */
	Ushahidi.Map.prototype.zoomToCurrentLocation = function(mapObj) {
		if (navigator.geolocation){
			navigator.geolocation.getCurrentPosition(showPosition);
		}
		function showPosition(position){
			if (!mapObj) mapObj = map._olMap;
