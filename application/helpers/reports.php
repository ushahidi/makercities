...
class reports_Core {
	...
	public static function validate(array & $post)
	{
		...
		// MakerCities: Future category lookup by name
		$category = ORM::factory('category')
			->where('category_title', $post->future_category . ' Level 1')
			->find_all();

		if (count($category) > 0) {
			$post->incident_category = Array($category[0]->id);
		} else {
		...
		if ( ! empty($post->incident_news))
		{
			...
			{
				if ($url == Kohana::lang('makercities.article_url')) {
					unset($post->incident_news[$key]);
					$url = '';
				}
		...
		if ( ! empty($post->incident_video))
		{
			...
			{
				if ($url == Kohana::lang('makercities.video_url')) {
					unset($post->incident_video[$key]);
					$url = '';
				}
	...
	public static function save_media($post, $incident)
	{
		...
		// c. Photos
		if ( ! empty($_FILES) && ! empty($post->incident_photo))
		{
	 ...
	 */
	public static function fetch_incidents($paginate = FALSE, $items_per_page = 0, $show_own_incidents = FALSE)
	{
		// Reset the paramters
		self::$params = array();
		...
		if ($show_own_incidents && Auth::instance()->logged_in()) {
			$logged_in_id = Auth::instance()->get_user()->id;
			array_push(self::$params, 'i.user_id = ' . $logged_in_id);
		}
	...
	/**
	 * Display Incidents in HTML for reports list, etc
	 */
	public function list_incidents_html($incidents) {
		ob_start();
		...
		foreach ($incidents as $incident)
		{
			...
			$rating = Reports_Controller::_get_rating($incident_id, 'original');
			...
			$rb_report = Array(
					'rb_report',
					'category-' . preg_replace("/-level-(\d)/", " level$1", str_replace(" ", "-", strtolower(Category_Lang_Model::category_title($category->id)))),
				);
			$rb_report = implode(' ', $rb_report);

			$stats = Incident_Model::get_stats($incident_id);
			if ((int) $incident->incident_phase > $stats['phase'])
				$rb_report .= ' level-up';
			$rating_plus = ($rating > $stats['rating']) ? $rating - $stats['rating'] : 0;
			$builds_plus = ($build_count > $stats['comments']) ? $build_count - $stats['comments'] : 0;
			?>
				<a href="<?php echo $incident_url; ?>" class="report_link">
				<div id="incident_<?php echo $incident_id ?>" class="<?php echo $rb_report; ?>">
					<div class="r_bars">
						<div class="r_supporters">
							<div class="support percentage" data-text="<?php echo $rating; ?>" title="<?php echo $rating; ?> <?php echo ($rating == 1) ? Kohana::lang('makercities.supporter') : Kohana::lang('makercities.supporters'); ?>"<?php if ($rating_plus > 0) : ?> data-change="+<?php echo $rating_plus; ?>"<?php endif; ?>>
								<div class="percentage-bar" style="width: <?php echo $rating_percent; ?>%;"></div>
							</div>
						</div>
						<div class="r_builds">
							<div class="builds percentage" data-text="<?php echo $build_count; ?>" title="<?php echo $build_count; ?> <?php echo ($build_count == 1) ? Kohana::lang('ui_main.comment') : Kohana::lang('ui_main.comments'); ?>"<?php if ($builds_plus > 0) : ?> data-change="+<?php echo $builds_plus; ?>"<?php endif; ?>>
	...
	public function get_city_name($city_id) {
		if ($city_id) {
			...
			$city_name = Kohana::lang('makercities.unknown_city');
		}
