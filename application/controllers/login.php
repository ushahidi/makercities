...
class Login_Controller extends Template_Controller {
		...
		elseif ($_POST AND isset($_POST["action"]) AND $_POST["action"] == "new")
		{
			...
			if ($post->validate())
			{
				...
				else
				{
					...
					// Log them in automatically and redirect to main site
					$user = $auth->login($post->email, $post->password, TRUE);

					setcookie('just_registered', '1');
	...
	public function facebook()
	{
		...
		if ($fb_user)
		{
			try
			{
				...
				if ($openid_user->loaded AND $openid_user->user)
				{
					...
					// Exists Redirect to Dashboard
					$redirect = $auth->get_user()->dashboard();
				}
				else
				{
					...
					if ( ! isset($new_openid["email"]) OR empty($new_openid["email"]))
					{
						$openid_error = TRUE;
						$message_class = 'login_error';
						$message = "User has not been logged in. No Email Address Found.";
					...
					else
					{
						...
						if ($user->email_exists($new_openid["email"]))
						{
							$openid_error = TRUE;
							$message_class = 'login_error';
							$message = $new_openid["email"] . " is already registered in our system.";
		...
		// Message to user
		if ($message) $this->session->set_flash('message', $message);
