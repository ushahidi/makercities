...
class Main_Controller extends Template_Controller {
	...
	public function __construct()
	{
		...
		$this->template->header->header_nav->my_submitted_futures = '';
		if ( isset(Auth::instance()->get_user()->id) )
		{
			...
			$incidents = reports::fetch_incidents(false, 0, true);
			$this->template->header->header_nav->my_submitted_futures = reports::list_incidents_html($incidents);
		}
    ...
    $this->template->content->all_categories = $categories;
		...
		$this->themes->js->marker_radius = ($marker_radius >=1 AND $marker_radius <= 20 )
		    ? $marker_radius
		    : 5;
