...
class Settings_Controller extends Admin_Controller {
			...
			if ($post->validate() AND $files->validate(FALSE))
			{
				...
				else
				{
					...
					if ($filename)
					{
						...
						Image::factory($filename)->resize(100,100,Image::HEIGHT)
	...
	*/
	...
	/**
	 * Handles MakerCities Settings
	 */
	public function makercities()
	{
		$this->template->content = new View('admin/settings/makercities');
		$this->template->content->title = Kohana::lang('ui_admin.settings');
		...
		$form = array(
			...
			'certified_threshold' => '',
			'supporter_value' => '',
			'proposal_comment_value' => '',
			'active_future_comment_value' => '',
			'certified_future_value' => '',
		);
		//	Copy the form as errors, so the errors will be stored with keys
		//	corresponding to the form field names
		$errors = $form;
		...
		if ($_POST)
		{
			...
			$post->add_rules('proposal_length', 'numeric', 'length[1,3]');
			$post->add_rules('active_threshold', 'numeric', 'length[1,3]');
			$post->add_rules('certified_threshold', 'numeric', 'length[1,3]');
			$post->add_rules('supporter_value', 'numeric', 'length[1,10]');
			$post->add_rules('proposal_comment_value', 'numeric', 'length[1,10]');
			$post->add_rules('active_future_comment_value', 'numeric', 'length[1,10]');
			$post->add_rules('certified_future_value', 'numeric', 'length[1,10]');
		...
		else
		{
			...
			// Retrieve Current Settings
			$form = array(
				'proposal_length' => $settings['proposal_length'],
				'active_threshold' => $settings['active_threshold'],
				'certified_threshold' => $settings['certified_threshold'],
				'supporter_value' => $settings['supporter_value'],
				'proposal_comment_value' => $settings['proposal_comment_value'],
				'active_future_comment_value' => $settings['active_future_comment_value'],
				'certified_future_value' => $settings['certified_future_value'],
			);
		}
	...
	public function update_cities($country_id = 0)
	{
		...
		if ($country->loaded)
		{
			...
			$country_url = $base_url."countryInfoJSON?country=%s&username=makercities";
			...
			if (($response = $client->execute()) !== FALSE)
			{
				...
				// 
				$cities_url = $base_url."citiesJSON?north=%s&south=%s&east=%s&west=%s&username=makercities&maxRows=50";
