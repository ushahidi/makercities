...
class Json_Controller extends Template_Controller {
	...
	protected function geojson($type)
	{
		...
		if (Kohana::config('settings.default_map_all_icon_id'))
		{
			...
			$icon = url::convert_uploaded_to_abs($icon_object->media_link);
		}
    ...
    $original_color = $color;
      ...
      if (isset($marker->category_color))
      {
        $color = $marker->category_color;
      }
      elseif($marker->category_color == null) 
      {
        $color = $original_color;
      }
       
      if (isset($marker->category_image))
      {
        $icon = preg_replace('/\/[^\/]*$/', "/".$marker->category_image, $icon);
      }
      elseif($marker->category_image == null) 
      {
        $icon = $original_icon;
      }
