...
class Incident_Model extends ORM {
	 ...
	 * Sets incident stats
	 * @param int $incident_id
	 * @param Array $arr of stats
	 * @return bool
	 */
	public static function set_stats($incident_id,$arr)
	{
		$incident = ORM::factory('incident',$incident_id);
		$incident->incident_stats = json_encode($arr);
		return $incident->save();
	}

	/**
	 * Gets incident stats and returns as Array
	 * @param int $incident_id
	 * @return Array
	 */
	public static function get_stats($incident_id)
	{
		$incident = ORM::factory('incident')->find($incident_id);
		$stats = $incident->incident_stats;
		$stats = ($stats == '') ? Array('phase' => 0, 'rating' => 0, 'comments' => 0) : json_decode($stats, true);
		return $stats;
	}
